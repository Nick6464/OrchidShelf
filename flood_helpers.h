...
void set_next_cycle_time(int bin_num, int current_time) {
  // Use the individual bin's cycle interval
...
float calculate_countdown_hours(int pump_num) {
  ...
  // If next cycle time is 0, calculate based on individual bin schedule
  if (next_cycle_time == 0) {
    ...
    if (schedule_mode == 0) {
      ...
      // Calculate hours until next scheduled run
      int hours_until = interval_time - current_hour;
      if (hours_until <= 0) hours_until += 24;
      
      // If we're past the interval, it should run today
      if (days_since >= (int)interval_days) {
        return hours_until;
      } else {
        // Calculate days until next interval
      ...
      // Parse comma-separated times and find next time
  ...
  return 0.0; // Cycle is due now or overdue
}
